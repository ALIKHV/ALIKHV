...
database:del("lock_note:ALIKHV"..msg.chat_id_..bot_id,"ok")
  ...
  local tsX_o = database:get("gamee:ALIKHV"..msg.chat_id_..bot_id)
  if not tsX_o then
  tsX000("lock",msg,"☑┇تم تعطيل الالعاب")
  else
  tsX000("lock",msg,"☑┇تم تعطيل الالعاب")
...
send(msg.chat_id_, msg.id_, 1, "☑️┊تم تفعيل البوت خدمي", 1, 'html')
end
...
return false
  ...
  local msg = data.message_
    ...
    if (text and text == 'تعين start' and  tonumber(msg.sender_user_id_) == tonumber(sudo_add) ) then 
      ...
      database:set("kali:addo"..msg.sender_user_id_, "yes")
      return false
    end  
    if text then 
    local kali = database:get("kali:addo"..msg.sender_user_id_)
    if kali == 'yes' then 
      send(msg.chat_id_, msg.id_, 1, "☑️┇تم حفظ كليشه start", 1, 'md')
      database:del("kali:addo"..msg.sender_user_id_)
      database:set("kali:start0"..bot_id, text)
      return false
    end
    end
...
tsX000("prore",msg,"☑┊بالتأكيد تم رفعه عضو مميز") 
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┊لا تستطيع حظر \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
   ...
   if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"☑┊بالتأكيد تم فك حظره من البوت")
     ...
     database:srem(hash, result.sender_user_id_)
       tsX000("prore",msg,"☑┊تم فك حظره من البوت")
   end
  end
  ...
  send(msg.chat_id_, 0, 1, '↙️┊رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if text:match("^فك الحظر @(.*)$") and is_mod(msg) then
...
if kali_add_sudo then var = true end return var
    ...
    if #sudo_kali == 0 or #sudo_kali == 1 and tonumber(sudo_kali[i]) == tonumber(sudo_add) then
    kali = kali.."\n ⚠️┇لا يوجد مطورين"
    end
    if #sudo_kali == 1 and tonumber(sudo_kali[1]) == tonumber(sudo_add) then
    kali = kali.."\n ⚠️┇لا يوجد مطورين"
    end
    send(msg.chat_id_, msg.id_, 1, kali, 1, 'md')
    end
...
if text == 'كيناي' then
moody = "نِٰـِۢعِٰـِۢمِٰـِۢ تِٰـِۢفٰ͒ـِۢضِٰـِۢلِٰـِۢ  ⁞✦⁽☻🔥₎“ٰۦ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'شلونكم' then
...
if text == 'بوت' then
moody = "آسِٰـِۢمِٰـِۢيِٰـِۢ ڪِٰـِۢيِٰـِۢنِٰـِۢآيِٰـِۢ - ⁽🙆‍♂️✨₎ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'اريد بوت' then
moody = "راسل المطور @ieoooo"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'لتزحف' then
...
if text == 'تعطيل الالعاب' and is_owner(msg) then  
